...
public class NurseCreateMessageController implements Initializable
{
    String staffID = "";
    ...
    @FXML
    private ComboBox<Patient> sendToPatientList;
    @FXML
    ...
    //get a list of all patients that a nurse can message
    public void loadRecipientList() throws SQLException {
        ...
        sendToPatientList.itemsProperty().setValue(patients);

        sendToPatientList.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
    ...
    {
        sendToPatientList.getSelectionModel().select(index);
    }
    ...
    public String validateInput()
    {
        ...
        // Validate recipient
        if (sendToPatientList.getValue() == null)
        {
    ...
    public void onSendMsg(ActionEvent event) throws IOException, SQLException
    {
        ...
        if(validRegistrationInput == null || validRegistrationInput.trim().isEmpty()) {
            try {
                ...
                pstmt.setString(2, sendToPatientList.getValue().getId());
